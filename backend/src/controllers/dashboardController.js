...
exports.getDashboard = async (req, res) => {
  try {
    ...
    const [
      ...
      // Daily quote trend last 30 days
      Quote.aggregate([
    ...
    // Average rating
    const reviews = await Review.find({ approved: true }).select('rating');
    const avgRating = reviews.length
      ? (reviews.reduce((a,r) => a+r.rating, 0) / reviews.length).toFixed(1)
      : 0;

    // Recent activity (last 10 quotes + contacts merged)
